...
msg = require('../build/Release/msg');

MessageQueue = function(key, perms, options) {
  ...
  this.on('newListener', function(event) {
    if (event === 'data') {
      setImmediate(function(){return that._beginReceive()});
    }
  });
...
MessageQueue.open = function(key, perms){return new MessageQueue(key, perms)};
...
MessageQueue.prototype._beginReceive = function() {
  ...
  this._receive(options, function(err, data){
    if (err) {
      this.emit('error', err);
      return;
    }
...
function formatFlags(perms) {
  uPerms = (perms & 0x006);
  gPerms = (perms & 0x060) >> 1;
  oPerms = (perms & 0x600) >> 2;
